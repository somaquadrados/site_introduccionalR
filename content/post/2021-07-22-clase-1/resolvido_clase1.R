...
total <- sum(Anopheles_sp, Lutzomyia_sp, Aedes, Desconocido)

(Lutzomyia_sp/total)*100

## 4 - Si eliminamos los mosquitos desconocidos, ¿cuál es la cantidad total
##     muestreada? ¿Y cuál es el porcentaje de Aedes?
total <- sum(Anopheles_sp, Lutzomyia_sp, Aedes)
...
#   3. un área rural. 
#
# De esta manera, cree un factor que repita 12 veces el nombre de cada 
# ubicación de recolección (= 1 año de muestreo).

reco <- as.factor(rep(c("PNI", "antro", "rural"), 12))
reco
...
# Con rbind --
p1 <- c(38, 37.9, 37.3, 37.2, 36.9)
p2 <- c(37.9, 37.6, 37.1, 36.8, 36)
p3 <- c(38.2, 38, 37.8, 37.2, 36.8)

med <- rbind(p1, p2, p3)
med

# Con matrix
med <- matrix(c(38, 37.9, 37.3, 37.2, 36.9,
                37.9, 37.6, 37.1, 36.8, 36,
                38.2, 38, 37.8, 37.2, 36.8), nrow = 3)

colnames(med) <- c("5m", "10m", "15m", "20m", "25")
rownames(med) <- c("P1", "P2", "P3")
med
...
acc <- c(396, 400, 236, 123, 238, 632)

tabla <- data.frame(loc, tiempo, poblacion, acc)
tabla
...
installed.packages() # array de datos

"vegan" %in% rownames(installed.packages()) # package vegan instalado?
