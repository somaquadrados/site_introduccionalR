...
hospital2 <- hospital[, c("Genéro", "Edad", "Barrio", "Médico")]
...
#     estas personas.
hospital3 <- hospital2[hospital2$Genéro == "F",]
...
hospital3$Búsqueda_média <- c("Tratamiento hormonal", "Problemas gástricos",
                              "Sospecha de dengue")
hospital3

# 4 - Cambie la clase de variable "edad" para numeric.
hospital3$Edad <- as.numeric(hospital3$Edad)
class(hospital3$Edad)
...
table(c(loc1, loc2, loc3) < 20)

# 3 - ¿Alguna de las ubicaciones tiene los valores [10, 20, 22]?
table(c(loc1, loc2, loc3) == c(22, 20, 10))
...
# • El 📦 magrittr oferece un operador que hace que su código sea más legible: 
#   el pipe (%>%).
...
tidy_ej <- read_xlsx("tidy_ej.xlsx") 

## DICA:
library(janitor)
tidy_ej <- read_xlsx("tidy_ej.xlsx") %>% clean_names()
names(tidy_ej)

## 2 - ¿Faltan datos en la tabla tidy_ej? Si es así, reemplace los valores 
##     faltantes con 0. Tip: datos %>% is.na() %>% table()
tidy_ej %>% is.na() %>% table()
tidy_ej %>% mutate_all(~replace(., is.na(.), 0))

## 3 - Separe la columna "departamento - año" en dos columnas.
tidy_ej %>% separate(departamento_ano, c("departamento", "ano"))
...
tidy_ej %>% pivot_longer(c(pni, urbano, rural), names_to = "areas",
                         values_to = "numero_acc")

## 5 - Guarde todo lo que ha hecho en una nueva table tibble llamada "tidy_ej2".
tidy_ej2 <- tidy_ej %>% 
  mutate_all(~replace(., is.na(.), 0)) %>%
  separate(departamento_ano, c("departamento", "ano")) %>%
  pivot_longer(c(pni, urbano, rural), names_to = "region",
               values_to = "numero_acc")
